...
def custom_plot(
    ...
    axs[0, 0].plot(time, T_air, label="T_air")
    axs[0, 0].plot(time, lb_list, "--", label="Lower Bound")
    axs[0, 0].plot(time, ub_list, "--", label="Upper Bound")
    axs[0, 0].set_title("Indoor Air Temperature")
    axs[0, 0].legend()

    axs[0, 1].plot(time, sat_list, label="SAT")
    axs[0, 1].plot(time, zat_list, label="ZAT")
    axs[0, 1].set_title("SAT and ZAT Setpoints")
    axs[0, 1].legend()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{idx}.png"), dpi=300)
    plt.show()
...
def main():
    ...
    policy_file = (
        "/Users/zhangyizhong/Desktop/meta_rl-develop_10/"
        "ddpg/ddpg_adapted.pt"
    )
