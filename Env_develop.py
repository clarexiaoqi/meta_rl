...
class ContinuousBuildingControlEnvironment(gym.Env):
    ...
    def __init__(
        ...
        lb_set=22.0,
        ub_set=24.0,
        # day/night comfort bounds
        night_lb_set=15.0,
        night_ub_set=28.0,
        mode_deadband=0.0,
        alpha=0.3,
        # reward normalization references
        E_ref=60.0,      # kWh
        T_ref=8.0,       # °C
        # action bounds
        SAT_low=12.8,
        ...
        # Comfort bounds
        # Daytime uses the original tight bound, nighttime uses the previous loose bound.
        self.day_lb = float(lb_set)
        self.day_ub = float(ub_set)
        self.night_lb = float(night_lb_set)
        self.night_ub = float(night_ub_set)

        # keep old names for backward compatibility in external scripts
        self.lb = self.day_lb
        self.ub = self.day_ub

        # Reward weight and normalization references
    ...
    def step(self, a_t):
        ...
        # comfort exceed with day/night bounds
        ...
        # -----------------------------
        # Scaled reward
        # Same scaling style as the previous environment:
        #   energy term / E_ref (default 60 kWh)
        #   temperature exceed term / T_ref (default 8 °C)
        # -----------------------------
        ...
        info = {
            ...
            # backward-compatible keys for offline_test.py
            "lb": float(comfort_lb),
            "ub": float(comfort_ub),
