...
def ddpg_torch(
    ...
    best_critic = deepcopy(critic)
    best_target_actor = deepcopy(target_actor)
    best_target_critic = deepcopy(target_critic)
    ...
    if safety_return_threshold is None:
        print(">>> Safety return threshold: will be set to 1.5 * first episode return")
    else:
    ...
    for t in range(steps):
        ...
        if d:
            ...
            # =========================
            # Initialize safety threshold from first episode
            # Because return is usually negative, 1.5 * first return
            # means allowing about 50% worse performance than the initial policy.
            # =========================
            if first_episode:
                if safety_return_threshold is None:
                    safety_return_threshold = 1.5 * ep_return

                first_episode = False

                print(
                    f">>> Safety threshold initialized: "
            ...
            if (
                t > warmup_steps
                and safety_return_threshold is not None
                ...
                critic.load_state_dict(
                    best_critic.state_dict()
                )

                target_actor.load_state_dict(
                    best_target_actor.state_dict()
                )

                target_critic.load_state_dict(
                    best_target_critic.state_dict()
                )
    ...
    return {
        ...
        "safety_return_threshold": safety_return_threshold,
    }