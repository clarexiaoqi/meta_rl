...
def soft_update_ddpg_to_ppo(ddpg_actor, ppo_actor, alpha=0.1):
    ...
    for src, dst in zip(ddpg_layers, ppo_layers):
        dst.weight.data.copy_(
            (1.0 - alpha) * dst.weight.data
            + alpha * src.weight.data
        )

        dst.bias.data.copy_(
            (1.0 - alpha) * dst.bias.data
            + alpha * src.bias.data
        )
...
def train_multi_env(env_list, model, optimizer, device):
    ...
    # =====================================================
    # Shared DDPG critic
    # This critic will be reused across all DDPG adaptations.
    # =====================================================
    obs_dim = int(np.prod(env_list[0].observation_space.shape))
    act_dim = int(np.prod(env_list[0].action_space.shape))

    shared_critic = Critic(obs_dim, act_dim).to(device)
    shared_target_critic = Critic(obs_dim, act_dim).to(device)
    shared_target_critic.load_state_dict(shared_critic.state_dict())

    shared_critic_opt = optim.Adam(
        shared_critic.parameters(),
        lr=SHARED_CRITIC_LR,
    )

    print(">> Shared DDPG critic initialized")
    ...
    for update in range(1, num_updates + 1):
        ...
        # =========================
        # DDPG adaptation with shared critic
        # =========================
        adapted_actor, adapted_return = ddpg_adapt(
            ...
            shared_target_critic=shared_target_critic,
            shared_critic_opt=shared_critic_opt,
        )

        ppo_eval_return = eval_ppo(
            env,
            model,
            device,
        )
    ...
    # =========================
    # Save shared DDPG critic
    # =========================
    torch.save(
        {
            "shared_critic": shared_critic.state_dict(),
            "shared_target_critic": shared_target_critic.state_dict(),
            "shared_critic_lr": SHARED_CRITIC_LR,
        },
        os.path.join(SAVE_DIR, "shared_ddpg_critic.pth"),
    )

    print(">> Saved shared DDPG critic")
    ...
    plt.savefig(
        os.path.join(SAVE_DIR, "meta_training_curve.pdf"),
        dpi=300,
        bbox_inches="tight",
    )

    plt.savefig(
        os.path.join(SAVE_DIR, "meta_training_curve.png"),
        dpi=300,
        bbox_inches="tight",
    )
...
if __name__ == "__main__":
    ...
    env_list = create_env_list_from_csv(
        EnvClass,
        CSV_FILE,
        n=100,
    )
