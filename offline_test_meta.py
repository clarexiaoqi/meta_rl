...
def custom_plot(
    ...
    ax_temp.plot(time_plot, T_air, linewidth=2.2,
                 label="Indoor Air Temperature")
    ax_temp.plot(time_plot, lb_list, "--", linewidth=1.1,
                 alpha=0.45, label="Lower Bound")
    ax_temp.plot(time_plot, ub_list, "--", linewidth=1.1,
                 alpha=0.45, label="Upper Bound")
    ax_temp.set_title("Indoor Air Temperature", fontsize=13,
                      fontweight="bold", pad=10)
    ax_temp.set_ylabel("Temperature (°C)", fontsize=12)
    ax_temp.legend(fontsize=7.5, loc="lower right",
                   framealpha=0.65, frameon=True)
    ...
    ax_ctrl.set_title("Control Inputs (SAT and ZAT)", fontsize=13,
                      fontweight="bold", pad=10)
    ...
    ax_out.set_title("Outdoor Air Temperature", fontsize=13,
    ...
    ax_solar.set_title("Solar Heat Gain", fontsize=13,
                       fontweight="bold", pad=8)
...
def test_policy(
    ...
    actor.model.load_state_dict(actor_state)
    actor.eval()

    print(">>> Loaded meta policy: final_actor.pth")
    ...
    with torch.no_grad():
        while True:
            if not done:
                ...
                action_val = actor(obs_t).cpu().numpy()[0]
                ...
                penalty_list.append(penalty_list[-1] + penalty_step)
...
def main():
    ...
    pd.DataFrame(d_profile).to_csv(
        "results_meta_policy_profile.csv",
        index=False,
    )

    print("Tested actor: meta_policy_final_actor")
