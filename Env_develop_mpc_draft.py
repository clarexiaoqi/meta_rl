...
class ContinuousBuildingControlEnvironment(gym.Env):
    ...
    def __init__(
        ...
        # Optional fixed weather forecast used only by the MPC prediction env.
        # The real environment should leave this as None.
    ...
    def set_forecast_override(self, forecast):
        """
        Set a fixed weather forecast trajectory for MPC prediction.

        Parameters
        ----------
        forecast : dict or None
            Mapping from absolute simulation time in hours to a dictionary
            containing at least:

                {
                    time_hour: {
                        "T_out": predicted outdoor temperature,
                        "Qsg": predicted solar gain,
                    }
                }

            Extra fields are allowed and ignored by the environment.

        Notes
        -----
        - Only T_out and Qsg are overridden.
        - Qint, Hour, and T_cor still come from the original dataset/schedule.
        - The real environment should never call this method.
        - The same fixed forecast should remain active for the entire MPC solve.
        """
        if forecast is None:
            self.forecast_override = None
            return
        ...
        for time_hour, values in forecast.items():
            if "T_out" not in values or "Qsg" not in values:
                raise KeyError(
                    "Each forecast entry must contain both 'T_out' and 'Qsg'."
                )

            key = round(float(time_hour), 8)
            cleaned[key] = {
                "T_out": float(values["T_out"]),
                "Qsg": max(float(values["Qsg"]), 0.0),
            }

        self.forecast_override = cleaned

    def clear_forecast_override(self):
        """Remove the temporary MPC weather forecast."""
        self.forecast_override = None

    def _get_disturbance_at_time(self, time_hour):
        """
        Read exogenous inputs at the requested absolute simulation time.

        By default, all disturbances come from the original dataset.

        When forecast_override is active, only T_out and Qsg are replaced by
        the fixed forecast values. Qint, Hour, and T_cor continue to come from
        the original dataset/schedule.
        ...
        idx = min(max(int(time_hour * 2), 0), len(self.data) - 1)
        ...
        if self.forecast_override is not None:
            key = round(time_hour, 8)
            forecast_values = self.forecast_override.get(key)

            if forecast_values is not None:
                disturbance["T_out"] = float(forecast_values["T_out"])
                disturbance["Qsg"] = max(float(forecast_values["Qsg"]), 0.0)
